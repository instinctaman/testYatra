...
(function () {
  ...
  $(".add-conditional-requirement-info").on(
    "click",
    ".trash-icon",
    function () {
  ...
  $(".add-common-reasons-for-rejection-info").on(
    "click",
    ".trash-icon",
    function () {
  ...
  $(".form-info").each(function () {
    ...
    if (checkIn.length) {
      ...
      checkIn.closest(".form-item").find("p.fs-12").text(today.format("dddd"));
  ...
  document.addEventListener("click", function (e) {
    if (e.target && e.target.classList.contains("apply-btn")) {
      ...
      // Get input values
      const adultInput = dropdown.querySelector(
        '.input-number[data-type="adult"]'
      );
      const childrenInput = dropdown.querySelector(
        '.input-number[data-type="children"]'
      );
      const infantInput = dropdown.querySelector(
        '.input-number[data-type="infant"]'
      );
      const roomInput = dropdown.querySelector(
        '.input-number[data-type="room"]'
      );
      ...
      // Cabin class
      const cabinClassInput = dropdown.querySelector(
